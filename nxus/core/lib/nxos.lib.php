...
function getNexus($idelem1,$type_elem1,$idelem2='',$type_elem2='',$type_rel=0,$status=1) {
	...
	if ($resql) {
		...
		while ($row = $db->fetch_array($resql))  {
			...
			//				  0		1		2		3		  4			5		6		 7
			$rows[$i] = array(23,$row[0], $row[1], $row[2], $row[3], $row[4], $row[5], $row[6],
			//						8		9		10			11		12		13			14
								$row[7], $row[8], $row[9], $row[10], $row[11], $row[12],$row[13],
			//						15		16			17		 18			19		20		21
								$row[14], $row[15], $row[16], $row[17], $row[18],$row[19],$row[20],
			//						22		23
								$row[21], $row[22]); //el ultimo elemento del arreglo es el numero de campos totales de la consulta
			$i++;
		}
...
function getNxusUrl($id,$ref,$table,$file) {
	...
	if ($resql) {
		...
		print "Error al tratar de obtener el URL del objeto. Intente de nuevo o Contacte al administrador: ".$sql; exit;
	}
	return $lien;
}
...
function listNxusMovs($idelem, $titulos, $range=null, $rens=null, $fieldcnds=null, $cnds=null, $field=null, $table=null, $list=null, $fieldlist=null,$val=null, $searchs=null) {
	...
	$html.= '<tr>';
	$html.= '<td align="left" bgcolor="#3B5998" style="color:white;" style="font-style:bold" colspan="'.$colspan.'">NEXOS Movimientos</td>';
	$html.= '</tr>';
	$html.= '<tr class="liste_titre">';
	foreach($titulos as $titulo) {
		$html.=	'<td align="center" style="font-weight:bold">'.$titulo.'</td>';
	}
	$html.=	'<td align="center" style="font-weight:bold">Movs.</td>';
	$html.= '</tr>';
	$var = true;
	$i=0;
	foreach($rens as $ren) {
		$var=!$var;
		$html.= "<tr $bc[$var]>";
		$movs = getNxusMov($idelem,$field,$table,array($fieldcnds[$i]),array($cnds[$i]),$list,$fieldlist,$val,$searchs[$i]);
		$html.=	'<td align="center" style="font-weight:bold">'.$ren.'</td>';
		$html.=	'<td align="center" style="font-weight:bold">'.$movs.'</td>';
		$html.= "</tr>\n";
		$i++;
	}
...
function getNxusMov($idelem, $field, $table, $fieldcnds=null, $cnds=null, $list=null, $fieldlist=null,$val=null,$search=null) {
	
	global $db;

	$sql = "SELECT rowid FROM ".MAIN_DB_PREFIX.$table;
	$sql.= " WHERE ".$field." = '".$idelem."'";
	$i=0;//asigno un indice
	foreach($fieldcnds as $fieldcnd) {
		$sql.= " AND ".$fieldcnd." = '".$cnds[$i]."'";
		$cnd = $cnds[$i];
		$i++;
	}
	//print $sql;//exit;
	$resql = $db->query($sql);
	if ($resql) {
		$rows = $db->fetch_array($resql);
		$nums = $db->num_rows($resql);
		if($nums > 0) {
			if ($search) $lien = '<a href="'.DOL_URL_ROOT.$list.'?mode=1&'.$fieldlist.'='.$val.'&'.$search.'='.$cnd.'">'.$nums.'</a>';
			else $lien = '<a href="'.DOL_URL_ROOT.$list.'?mode=1&'.$fieldlist.'='.$val.'">'.$nums.'</a>';
			return $lien;
		}
		else return null;
	}

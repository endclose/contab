...
function exportSQLXls($sql, $fields_hide = 0, $fields_name = null, $report_name = null, $filename = null, $sep_ca = null, $stat_cols = null)
{
	...
	$i = 0;
	$c = 65;
	$ren = 6; //print $db->num_rows($result);print '<br>';
	...
	if ($stat_cols) {
		...
		$ren2++;
		$ref = $col . $ren2;
		$objPHPExcel->getActiveSheet()->setCellValue($ref, 'PROMEDIO:');
		$ren2++;
		$ref = $col . $ren2;
		$objPHPExcel->getActiveSheet()->setCellValue($ref, 'MÁX:');
		$ren2++;
		$ref = $col . $ren2;
		$objPHPExcel->getActiveSheet()->setCellValue($ref, 'MÍN:');
		...
		foreach ($stat_cols as $stat_col) {
			...
			$ren3++;
			$ref = $col . $ren3;
			$suma = '=SUM(' . $col . '6:' . $col . ($ren - 1) . ')';
			$objPHPExcel->getActiveSheet()->setCellValue($ref, $suma);
			$ren3++;
			$ref = $col . $ren3;
			$prom = '=AVERAGE(' . $col . '6:' . $col . ($ren - 1) . ')';
			$objPHPExcel->getActiveSheet()->setCellValue($ref, $prom);
			$ren3++;
			$ref = $col . $ren3;
			$max = '=MAX(' . $col . '6:' . $col . ($ren - 1) . ')';
			$objPHPExcel->getActiveSheet()->setCellValue($ref, $max);
			$ren3++;
			$ref = $col . $ren3;
			$min = '=MIN(' . $col . '6:' . $col . ($ren - 1) . ')';
			$objPHPExcel->getActiveSheet()->setCellValue($ref, $min);
...
function exportBalanzaXls($ejercicio, $periodo, $fields_hide = 0, $fields_name = null, $report_name = null, $filename = null, $sep_ca = null, $stat_cols = null)
{
	...
	switch ($periodo) {
		...
		case 8:
			...
			break;
		case 9:
			$mes_periodo = "Septiembre";
			break;
		case 10:
			$mes_periodo = "Octubre";
			break;
		case 11:
			$mes_periodo = "Noviembre";
			break;
		case 12:
			$mes_periodo = "Diciembre";
			break;
	}

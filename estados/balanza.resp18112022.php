...
$formconfirm = $form->form_confirm($_SERVER['PHP_SELF'] . '?ejercicio=idejercicio&periodo=idperiodo&mode=1', $aviso, $text, 'add_mov', $formquestion, "yes", 1,210,300);
}
...
/*        print '<input type="image" value="button_search" src="'.DOL_URL_ROOT.'/media/imgs/interface/zoom-2c.png" name="button_search" value="'.dol_escape_htmltag($langs->trans("Search")).'" title="'.dol_escape_htmltag($langs->trans("Search")).'">';print '<img src="'.DOL_URL_ROOT.'/media/imgs/interface/substract-2d.png">';
    ...
    //    print '</td>';
        ...
        while ($i < $db->num_rows($result))
        {
            ...
            $movs = getNxusMov($obj->rowid,'IdCuenta','contab_polizas_movs',array('Ejercicio'),array($ejercicio),'/contab/polizas/listmov.php','search_codigo',$obj->codigo);
            if ($movs) {
                print "<tr $bc[$var]>";

...
//				$sql.= $rw[0];
				...
				$result = $this->db->query($sql);
				if ($result) {
					$idm = $this->db->last_insert_id(MAIN_DB_PREFIX."movimientospoliza");
...
//consulta MySQL		
		$sql = "UPDATE ".MAIN_DB_PREFIX."polizas SET";
...
/*FUNCION PARA BORRAR LAS POLIZAS Y SUS MOVIMIENTOS */
	function borrarPoliza($id) {
		...
		$sql = 'DELETE FROM '.MAIN_DB_PREFIX.'polizas WHERE rowid = '.$id;
		$resql=$this->db->query($sql);
		if ($resql) {  //Si se pudo borrar la póliza ahora borramos la poliza de contpaq
			$sql = 'DELETE FROM '.MAIN_DB_PREFIX.'movimientospoliza WHERE IdPoliza = '.$id;
...
function borrarMovimiento($idmov) {
	...
	$sql = 'SELECT mp.IdPoliza, mp.IdCuenta FROM '.MAIN_DB_PREFIX.'movimientospoliza as mp WHERE mp.rowid = '.$idmov;
	$resql=$this->db->query($sql);
	$obj = $this->db->fetch_object($resql);
	$idpol = $obj->IdPoliza;
	$idcuenta = $obj->IdCuenta;
	//eliminamos el movimiento	
	$sql = 'DELETE FROM '.MAIN_DB_PREFIX.'movimientospoliza WHERE rowid = '.$idmov;
	$resql=$this->db->query($sql);
	if ($resql) {	//se elimino el movimiento exitosamente procedemos a recalcular los num movimientos que quedaron y reasignarlos
		$sql = 'SELECT mp.rowid FROM '.MAIN_DB_PREFIX.'movimientospoliza as mp WHERE mp.IdPoliza = '.$idpol;	//seleccionamos los movimientos que quedan de la poliza
		...
		if ($resql2) {
			if ($numov > 0) {		//despues de la eliminacion del movimiento quedaron mas de un registro procedemos al reasignacion del num de movimiento
				...
				$i=1;
				foreach ($rows as $row) {
					$sql = "UPDATE ".MAIN_DB_PREFIX."movimientospoliza SET NumMovto=".$i." WHERE rowid = ".$row; 
...
//$date							int: 1: se calculara a una fecha determinada 0: se calcula normal
	function getCargosAbonos($idcuenta=null, $periodo=null, $ejercicio=null) {
		...
		$sql.= " FROM ".MAIN_DB_PREFIX."movimientospoliza as mp";
...
// $date					date: fecha especifica a la que se obtendra el saldo
	function getSaldo($idcuenta=null, $ejercicio=null, $periodo=null, $date=null) {
		...
		if ($date) { //se obtendra el saldo de la cuenta en base al ultimo dia del periodo y ejercicio solicitado
			//	obtenemos el ultimo dia del mes
//			$dia = date("d",(mktime(0,0,0,$periodo+1,1,$ejercicio)-1));
//			$date = $ejercicio."/".$periodo."/".$dia;
			print $date; exit;
			
		}
		//					0			1			2			 3			4			5			6		  7
		$sql.= "SELECT mp.IdCuenta, mp.Periodo, mp.Ejercicio, mp.Fecha, mp.TipoPol, mp.Folio, mp.NumMovto, mp.saldo";
		$sql.= " FROM ".MAIN_DB_PREFIX."movimientospoliza as mp";
		$sql.= " WHERE mp.IdCuenta=".$idcuenta." AND mp.Periodo=".$periodo." AND mp.Ejercicio='".$ejercicio."'";
		$sql.= " ORDER BY mp.IdCuenta, mp.Periodo, mp.Ejercicio, mp.Fecha DESC , mp.TipoPol, mp.Folio DESC , mp.NumMovto";
		$sql.= " LIMIT 0,1"; //print $sql.'<br>';// exit;
		$resql = $this->db->query($sql);
		if ($resql) {
			$i=0;
			while ($row = $this->db->fetch_array($resql))  {
				$rows[$i] = array($row[0],$row[7]);
				$i++;
			}			
		}
		else return false;		
	...
	function ActualizaSaldosCuenta($idcuenta=null,$all=null) {
		...
		if ($resql) {
			...
			while ($row = $this->db->fetch_array($resql))  {
				...
				$sql = "UPDATE ".MAIN_DB_PREFIX."movimientospoliza SET";
				$sql.= " saldo=".$saldo_acum;

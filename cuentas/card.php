...
if ($mode==4 || $mode==5)	//procedimiento para el alta del registro
{
	...
	$_POST['codigo'] = trim($_POST['codigo']); $_POST['codigo'] = str_replace($caracteres_inval,"",$_POST['codigo']);
	$object->rowversion = $rowversion; $object->codigo = $_POST['codigo'];	$object->nombre = $_POST['nombre'];	$object->nomidioma = ''; $object->tipo = $_POST['idtipocta'];
	$object->active = $_POST['estado']; $object->nivel = $_POST['nivel']; $object->ctaefectivo = '0'; $object->FechaRegistro = date("Y/m/d"); $object->sistorigen = '11'; 
	$object->idmoneda = '1'; $object->digagrup = '0'; $object->idsegneg = '0'; $object->segnegmovtos = '0'; $object->afectable = $_POST['afectable']; $object->timestamp = '';
	$object->idrubro = '0'; $object->consume = '0'; $object->idagrupadorsat = '0'; $object->conceptosconsume = ''; $object->idpadre = $_POST['padre']; $object->nat = $_POST['nat'];
...
{
	//Datos cuenta
	print '<tr><td align="center" bgcolor="#3B5998" style="color:white;" style="font-style:bold"><h6>'.'DATOS CUENTA'.'&nbsp;</h6></td></tr>';
	...
	$datos_idactiva = array(0,1); $datos_nactiva = array('ACTIVA','INACTIVA');
	print '<td>';
	print select_simple('estado', $object->active,$datos_idactiva,$datos_nactiva,2); 
	print '</td></tr>';
	//Afectacion
	print '<tr><td align="center" bgcolor="#3B5998" style="color:white;" style="font-style:bold"><h6>'.'AFECTACIÓN'.'&nbsp;</h6></td></tr>';	
	print '<tr><td width="20%">'.'Tipo de Cuenta'.'</td>';	
	$names = array('etiqueta');
	print '<td>';
	print selectTipoCta($object->tipocta,'idtipocta');
	print '</td></tr>';
	$nat_ids = array(1,-1); $nat_names = array('Deudora','Acreedora');
	...
	$datos_idactiva = array(0,1); $datos_nactiva = array('No','Si');
	print '<td>';
	print select_simple('afectable', $object->afectable,$datos_idactiva,$datos_nactiva,2); 
	print '</td></tr>';
	//Ubicacion
	print '<tr><td align="center" bgcolor="#3B5998" style="color:white;" style="font-style:bold"><h6>'.'UBICACIÓN'.'&nbsp;</h6></td></tr>';	
	print '<tr><td width="20%">'.'Pertenece a '.'</td>';
	$names = array('nombre'); $fieldcnds = array('active','Afectable'); $cnds = array(0,0);
	print '<td>';select_cat('padre',MAIN_DB_PREFIX.'contab_cuentas',$object->idpadre,'codigo',$names,$fieldcnds,$cnds,'codigo','rowid'); print '</td></tr>';
...
//cfdi y conta electronica
//determinamos el numero de ejercicios fiscales que han trasncurrido desde el ejercicio inicial al ejercicio actual
...
$rens = $rows;
$cnds = $rows;
$rens[$i]="Periodo actual";
$fieldcnds[$i] = 'Periodo';
$searchs[$i]='search_periodo';
$cnds[$i] = $conf->global->PERIOD_CONTAB;
$list = '/custom/contab/polizas/listmov.php';
//Nexus movs
	print '<table class="noborder nohover" width="30%" align="left">';
	print listNxusMovs($object->id,array('Ejercicio'),$range,$rens,$fieldcnds,$cnds,'IdCuenta','contab_polizas_movs',$list,'search_codigo',$object->codigo, $searchs);
	print '</table>';
//
//Nexus movs
	print '<table class="noborder nohover" width="30%" align="right">';
	print $contab->listSaldosCuenta($object->id,$object->nat);
	print '</table>';
//

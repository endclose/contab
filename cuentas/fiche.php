...
if ($id) {		//existe un id
	...
	header("Location: ".DOL_URL_ROOT."/contab/index.php?mainmenu=contabilidad&leftmenu=contab");
	exit;	
}
...
if ($mode==4 || $mode==5)	//procedimiento para el alta del registro
{
	...
	$object->rowversion = $rowversion; $object->codigo = $_POST['codigo'];	$object->nombre = $_POST['nombre'];	$object->nomidioma = ''; $object->tipo = $_POST['tipocta'];
...
if ($mode==2 || $mode==3)
{
	...
	print '<td>';select_simple('estado', $object->active,$datos_idactiva,$datos_nactiva,2); print '</td></tr>';
	//Afectacion
	...
	print '<td>';select_cat('tipocta',MAIN_DB_PREFIX.'tiposcta_contpaq',$object->tipo,'abrev',$names,'','','abrev','abrev'); print '</td></tr>';
	$nat_ids = array(1,0); $nat_names = array('DEUDORA','ACREEDORA');
	print '<tr><td width="20%">'.'Naturaleza '.'</td>';
	print '<td>';select_simple('nat',$object->nat,$nat_ids,$nat_names,2); print '</td></tr>';	
	//Ubicacion
	...
	print '<td>';select_cat('padre',MAIN_DB_PREFIX.'c_cuentas_contab',$object->idpadre,'codigo',$names,$fieldcnds,$cnds,'codigo','rowid'); print '</td></tr>';
	print '<tr><td width="20%">'.'Afectable'.'</td>';
	$datos_idactiva = array(0,1); $datos_nactiva = array('No','Si');
	print '<td>';select_simple('afectable', $object->Afectable,$datos_idactiva,$datos_nactiva,2); print '</td></tr>';
	print '<tr><td width="20%">'.'Cuenta mayor'.'</td>';
	$datos_idactiva = array(1,2,3); $datos_nactiva = array('Si','No','Agrupacion');
	print '<td>';select_simple('nivel', $object->nivel,$datos_idactiva,$datos_nactiva,3); print '</td></tr>';

...
if ($mode == 3 && $modem == 2) {
	...
	if ($idcuenta) {
		...
		$poliza_mov = array(
			$poliza[1][0], $poliza[1][1], $poliza[1][2], $poliza[1][3], $poliza[1][4], $poliza[1][5],
		...
		if ($ejecutar > 0) {
			...
			print "Ocurrio un error al agregar el movimiento a la póliza. Intente de nuevo o contacte al administrador";
			exit;
		}
	}
...
if ($mode == 4 || $mode == 5)	//procedimiento para el alta del registro mode 4, procedimiento de actualizar registro mode 5
{
	...
	if ($fechapol == '') {
		...
		if ($ejecutar > 0) {
			...
			print "Ocurrio un error al contabilizar la póliza. Intente de nuevo o contacte al administrador";
			exit;
		}
...
if ($mode == 2) {  //si estamos en el modo de agregar cuenta eliminamos los $id del objeto $object exceptuando el de la cuenta actual
	unset($poliza);
	$poliza[1][9] = date('Y-m-d');
	$poliza[1][4] = 1;
	$poliza[1][3] = $conf->global->PERIOD_CONTAB;
	$poliza[1][2] = $conf->global->FISCAL_YEAR;
	$poliza[1][8] = '';
	$poliza[1][5] = '';
...
if ($mode == 2) {
	...
	print '<td>';
	selectTipoPol($poliza[1][4], 'idtipopol');
	print '</td></tr>';
